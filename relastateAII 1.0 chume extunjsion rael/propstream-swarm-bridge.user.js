...
(function () {
  ...
  const SELECTORS = {
    ...
    monitorButtons: [
      'button[data-testid*="monitor"]',
      'button[aria-label*="monitor" i]',
  ...
  function loadConfig() {
    return { ...DEFAULT_CONFIG, ...GM_getValue(STORAGE_KEYS.config, {}) };
  }
  ...
  const Discord = {
    ...
    async postAiHqAlert(content) {
      if (!state.config.discordAiHqWebhook) return;
      await this.post(state.config.discordAiHqWebhook, content);
    },
  };
  ...
  async function applyThresholdAlerts(scope) {
    ...
    for (const threshold of QUOTA_THRESHOLDS) {
      if (pct >= threshold && !state.thresholdState[scope].includes(threshold)) {
        ...
        await Discord.postAiHqAlert(`:warning: ${text}`).catch(() => undefined);
  ...
  async function guardCostBearingCommand(scope, commandCost = 1) {
    ...
    const remote = await PropStream.readQuotaCounters().catch(() => null);
    if (remote && typeof remote[normalized] === "number" && remote[normalized] <= 0) {
      throw buildBridgeError(
  ...
  const PropStream = {
    ...
    async save(payload) {
      ...
      for (const propertyId of payload.property_ids || []) {
        ...
        if (!row) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Property row not found in current view",
            item_ref: propertyId,
          });
          continue;
        }
        ...
        if (!saveButton) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Save button missing",
            item_ref: propertyId,
          });
          continue;
        }
        const before = Dom.text(saveButton).toLowerCase();
        ...
        items.push({ property_id: propertyId, status: "success" });
      }
      return {
    ...
    async exportList(payload) {
      ...
      await Dom.click(button);
      await randomizedDelay();
      const records = await this.attemptCaptureCsvRecords();
          ...
          : [
              {
                ...
                message: "Export was triggered but no downloadable file was captured",
    ...
    async attemptCaptureCsvRecords() {
      const csvLink = Array.from(document.querySelectorAll("a[href], button"))
        .find((node) => /(csv|xlsx|download)/i.test(node.href || Dom.text(node)));
      if (csvLink?.href) {
        try {
          const response = await gmRequest({
            method: "GET",
    ...
    async skipTrace(payload) {
      ...
      for (const propertyId of payload.property_ids || []) {
        ...
        if (!row) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Property row not found for skip trace",
            item_ref: propertyId,
          });
          continue;
        }
        ...
        if (!button) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Skip trace button missing",
            item_ref: propertyId,
          });
          continue;
        }
        await Dom.click(button);
        await delay(2_500);
        const data = this.extractSkipTraceContacts(propertyId);
    ...
    extractSkipTraceContacts(propertyId) {
      const blob = document.body.innerText || "";
      const phones = Array.from(
        new Set(blob.match(/\+?1?\s?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g) || []),
      ).slice(0, 10);
    ...
    async monitor(payload) {
      ...
      for (const propertyId of payload.property_ids || []) {
        ...
        if (!row) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Property row not found for monitor",
            item_ref: propertyId,
          });
          continue;
        }
        const button =
          row.querySelector(SELECTORS.monitorButtons.join(", ")) ||
          Dom.findButtonByText(/monitor|watch/i, row);
        if (!button) {
          errors.push({
            code: "DOM_SELECTOR_MISSING",
            message: "Monitor button missing",
            item_ref: propertyId,
          });
          continue;
        }
        await Dom.click(button);
        await delay(600);
        await incrementCounter("monitor", 1);
        items.push({ property_id: propertyId, status: "success" });
      }
      return {
        status: errors.length ? (items.length ? "partial" : "failure") : "success",
    ...
    async readQuotaCounters(forceNavigation = false) {
      ...
      if (Object.values(counters).every((value) => value === null)) {
        throw buildBridgeError(
          "DOM_SELECTOR_MISSING",
          "Quota counters not found on the current page",
        );
      }
      const normalized = {
        saves:
          counters.saves === null ? SOFT_CAPS.saves - (state.counters.saves || 0) : counters.saves,
        exports:
          counters.exports === null
            ? SOFT_CAPS.exports - (state.counters.exports || 0)
            : counters.exports,
        skip_trace:
          counters.skip_trace === null
            ? SOFT_CAPS.skip_trace - (state.counters.skip_trace || 0)
            : counters.skip_trace,
        monitor:
          counters.monitor === null
            ? SOFT_CAPS.monitor - (state.counters.monitor || 0)
            : counters.monitor,
      };
  ...
  function reconcileCounters(remoteRemaining) {
    ...
    if (drifted.length) {
      ...
      Discord.postAiHqAlert(`:warning: ${message}`).catch(() => undefined);
    ...
    try {
      ...
      switch (commandType) {
        ...
        case "MONITOR":
          ...
          result = await PropStream.monitor(envelope.payload);
          break;
        case "QUOTA_CHECK":
      ...
      if (code === "DOM_SELECTOR_MISSING") {
        await Discord.postAiHqAlert(
          `:rotating_light: DOM selector issue during ${commandType}. Relevant operation halted.\n\`\`\`\n${truncate(
            Dom.sanitizeDomStructure(),
            1500,
          )}\n\`\`\``,
        ).catch(() => undefined);
      }
  ...
  function mountPanel() {
    ...
    panel.innerHTML = `
      ...
      <label>AI HQ webhook <input id="pssb-discord-aihq-webhook" type="password" /></label>
  ...
  function bindPanelActions() {
    byId("pssb-save-config").addEventListener("click", () => {
      state.config = {
        ...
        discordAiHqWebhook: valueOf("pssb-discord-aihq-webhook"),
  ...
  function renderPanelValues() {
    const fields = {
      ...
      "pssb-discord-aihq-webhook": state.config.discordAiHqWebhook,
