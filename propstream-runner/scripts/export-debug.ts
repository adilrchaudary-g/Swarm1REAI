...
async function dismissModals(page: any) {
  await page.evaluate(`(function(){var btns=document.querySelectorAll("button");for(var i=0;i<btns.length;i++){if(/^(close|ok|done|got it|×|x)$/i.test((btns[i].textContent||"").trim())){var r=btns[i].getBoundingClientRect();if(r.width>0&&r.height>0&&r.y>50){btns[i].click();return true}}}return false})()`);
}

async function closeFilterPanel(page: any) {
  // Click the "Filters" button in the top toolbar to toggle off the panel
  const closed = await page.evaluate(`(function(){
    ...
    // Try clicking a close/X button inside the filter panel
    var closes = document.querySelectorAll("[class*='filter'] button[class*='close'], [class*='sidebar'] button[class*='close']");
    for (var i = 0; i < closes.length; i++) {
      var r = closes[i].getBoundingClientRect();
      if (r.width > 0 && r.height > 0) { closes[i].click(); return "closed-btn"; }
    }
    return false;
  })()`);
...
// Click Actions dropdown in the search results toolbar (right panel header)
  ...
  return page.evaluate(`(function(){
    ...
    for (var i = 0; i < btns.length; i++) {
      ...
      if (ownText.trim() === "Actions") {
        var r = btns[i].getBoundingClientRect();
        // The Actions button is in the right property panel header (x > 800, y < 200)
...
async function clickDropdownItem(page: any, label: RegExp): Promise<boolean> {
  ...
  return page.evaluate(`(function(){
    ...
    // PropStream uses DIVs with class 'dropdownItem' inside a 'dropdownCard'
...
async function getDropdownItems(page: any): Promise<string[]> {
  return page.evaluate(`(function(){
    var items = [];
    var els = document.querySelectorAll("[class*='dropdownItem'], [class*='dropdownCard'] > *, [class*='dropdown'] div, [class*='dropdown'] li, [class*='dropdown'] a, [class*='Dropdown'] div, [class*='menu'] li, [role='menuitem']");
    for (var i = 0; i < els.length; i++) {
      var ownText = "";
      for (var j = 0; j < els[i].childNodes.length; j++) {
        if (els[i].childNodes[j].nodeType === 3) ownText += els[i].childNodes[j].textContent;
...
async function runSignal(page: any, browser: any, zip: string, signal: string, signalToggle: string) {
  console.log(`\n${"=".repeat(60)}`);
  console.log(`SIGNAL: ${signal}`);
  console.log(`${"=".repeat(60)}`);

  // Navigate to search — dismiss any leftover modals first
  await page.goto("https://app.propstream.com/search", { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(3000);
  await dismissAlerts(page);
  await page.waitForTimeout(500);
  // Force-dismiss any modal overlays (from prior signal runs)
  ...
  // Expand Value & Equity, fill ranges
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-01-filters-set.png`) });

  // CLOSE the filter panel before interacting with search results
  await closeFilterPanel(page);
  await page.waitForTimeout(1500);

  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-02-filters-closed.png`) });

  // Wait for results to load (filters auto-apply)
  await page.waitForTimeout(3000);

  const count = await getResultCount(page);
  console.log(`  Found: ${count} properties`);

  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-03-results.png`) });

  if (count === 0) {
    console.log(`  SKIPPED: No results for ${signal}`);
    return null;
  }

  const maxProperties = Math.min(count, 3000);
  const listName = `harvest-${signal}-${Date.now()}`;
  let totalSaved = 0;

  // === PAGINATED SAVE: check 50 per page, save to same list, reload search for next page ===
  const totalPages = Math.min(Math.ceil(maxProperties / 50), 60);

  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    console.log(`  Page ${pageNum}/${totalPages}: selecting checkboxes...`);
    ...
    if (pageNum === 1) {
      // Create new list — click innermost "(Create as New List)" option
      await page.evaluate(`(function(){
      ...
      // Select existing list from autocomplete dropdown
      await page.waitForTimeout(500);
      const selectedExisting = await page.evaluate(`(function(){
        var name = ${JSON.stringify(listName)};
        // First try: exact match in dropdown items (exclude "Create" option)
        ...
        // Second try: partial match (list names may include count suffix like "(50)")
        for (var i = 0; i < els.length; i++) {
      ...
      if (!selectedExisting) {
        // Fallback: click the first non-create dropdown option
        await page.evaluate(`(function(){
          var els = document.querySelectorAll("[class*='dropdown'] div, [class*='option'], li");
          for (var i = 0; i < els.length; i++) {
            var t = (els[i].textContent || "").trim();
            if (t.length > 5 && !/create/i.test(t) && !/select or type/i.test(t)) {
              var r = els[i].getBoundingClientRect();
              if (r.width > 0 && r.height > 0 && r.height < 50 && r.y > 200 && r.y < 500) {
                els[i].click();
                return t;
              }
            }
    ...
    // Click Save in modal (skip trace will be done separately via toolbar)
    await page.evaluate(`(function(){
    ...
    // Wait for save to complete, then reload search page for next batch
    // (DO NOT force-remove modal overlays — it breaks React)
    await page.waitForTimeout(5000);

    if (pageNum >= totalPages) break;

    // Reload the search page cleanly to get fresh state for next page
    await page.goto("https://app.propstream.com/search", { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(3000);
    await dismissAlerts(page);

    // Re-enter search location
    let zipReInput = null;
    ...
    // Close filter panel (must succeed or paginator will be blocked by overlay)
    await closeFilterPanel(page);
    await page.waitForTimeout(1000);
    // Force-close overlay if it persists
    await page.evaluate(`(function(){
    ...
    // Navigate to target page via JavaScript (avoids overlay blocking Playwright clicks)
    ...
    console.log(`    Navigated to page ${nextPage}: ${JSON.stringify(navResult)}`);
    await page.waitForTimeout(3000);
  }

  console.log(`  Total saved: ${totalSaved} properties to "${listName}"`);
  if (totalSaved === 0) return { signal, count, maxProperties, totalSaved, csvPath: null };

  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-06-after-save.png`) });

  // ============ NAVIGATE TO LIST ============
  console.log(`  Navigating to My Properties...`);
  await page.goto("https://app.propstream.com/property/group/0", { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
  await page.waitForTimeout(3000);
  await dismissAlerts(page);
  await page.waitForTimeout(2000);

  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-07-my-properties.png`) });

  // Find and click the list using PropStream's LeftPanel CSS-module selectors
  let groupId: string | null = null;
  const listFound = await (async (): Promise<boolean> => {
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (labelClicked) {
        ...
        if (m) {
          groupId = m[1];
          console.log(`  Navigating to /property/group/${groupId}`);
          await page.goto(`https://app.propstream.com/property/group/${groupId}`, { waitUntil: "domcontentloaded" });
          await page.waitForTimeout(4000);
        }
        return true;
      }
      // Scroll the LeftPanel sidebar
      await page.evaluate(`(function(){
        var panels = document.querySelectorAll('[class*="LeftPanel"], [class*="leftPanel"], [class*="propertyList"]');
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-08-list-page.png`) });

  const currentUrl = page.url();
  console.log(`  URL: ${currentUrl}`);
  if (!/property\/group\/[1-9]/.test(currentUrl)) {
    console.log(`  Not on a list page — skipping`);
    return { signal, count, maxProperties, totalSaved, csvPath: null };
  }

  // ============ SKIP TRACE ============
  // Select visible rows via header checkbox (Playwright mouse.click for AG-Grid)
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-09-skip-trace-modal.png`) });
  ...
  if (inProgress) {
    console.log(`  Skip trace in progress — waiting 90s then retrying...`);
    await dismissModals(page);
    await page.waitForTimeout(90000);
    await page.reload({ waitUntil: "domcontentloaded" });
    await page.waitForTimeout(5000);
    // Re-select rows and re-click Skip Trace
    const hdrCellsRetry = page.locator('.ag-header-cell[col-id="resultIndex"]');
    for (let hi = 0; hi < await hdrCellsRetry.count().catch(() => 0); hi++) {
      const box = await hdrCellsRetry.nth(hi).boundingBox().catch(() => null);
    ...
    const skipBtnsRetry = page.locator("button").filter({ hasText: /^Skip Trace$/ });
    for (let si = 0; si < await skipBtnsRetry.count(); si++) {
      const box = await skipBtnsRetry.nth(si).boundingBox().catch(() => null);
      if (box && box.width > 0) {
        await page.mouse.click(box.x + box.width / 2, box.y + box.height / 2);
        break;
      }
  ...
  // Fill "Name Your List" using coordinate-based input (Playwright locators miss this input)
  const stListName = `st-${signal}-${Date.now()}`;
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (r.width > 100 && r.height > 20 && r.y > 100 && r.y < 500) {
        if (ph.includes("list") || ph.includes("name") || ph.includes("enter")) {
          return { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height), ph: inputs[i].placeholder };
        }
      }
    }
    return null;
  })()`);
  console.log(`  Skip trace input: ${JSON.stringify(inputInfo)}`);
  ...
  if (!ordered) {
    console.log(`  Place Order failed — skipping skip trace`);
    await dismissModals(page);
  }

  // Wait for skip traces to process
  console.log(`  Waiting 90s for skip trace processing...`);
  await page.waitForTimeout(90000);

  // Reload list page to get fresh data with phone numbers
  if (groupId) {
    await page.goto(`https://app.propstream.com/property/group/${groupId}`, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(5000);
    await dismissAlerts(page);
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-10-after-skip-trace.png`) });

  // ============ EXPORT CSV ============
  // Select rows again (page was reloaded)
  ...
  // Click Actions (visible instance only)
  ...
  // Click "Export CSV" from dropdown
  ...
  if (await exportCsv.isVisible().catch(() => false)) {
    ...
    console.log(`  Export CSV not found in dropdown — trying fallback`);
    await clickDropdownItem(page, /export\s*csv/i);
  }

  const download = await downloadPromise;
  if (download) {
    csvPath = path.join(OUTPUT_DIR, `${signal}.csv`);
    ...
    await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-11-export-fail.png`) });
  }

  return { signal, count, maxProperties, totalSaved, csvPath };
}

async function handleSkipTraceModal(page: any) {
  await page.waitForTimeout(1000);
  // Toggle re-skip-trace if present
  ...
  // Click Place Order
  const ordered = await page.evaluate(`(function(){
    var btns = document.querySelectorAll("button");
    for (var i = 0; i < btns.length; i++) {
      if (/place order/i.test(btns[i].textContent) && !btns[i].disabled) {
        btns[i].click();
        return true;
      }
    }
  ...
  console.log(`  Place Order: ${ordered}`);
  await page.waitForTimeout(3000);
}
...
(async () => {
  ...
  const results: any[] = [];
  for (const [signal, toggle] of signals) {
    try {
      const r = await runSignal(page, browser, ZIP, signal, toggle);
      results.push(r);
    } catch (err) {
      console.error(`  ERROR on ${signal}: ${err instanceof Error ? err.message : err}`);
      await page.screenshot({ path: path.join(OUTPUT_DIR, `${signal}-ERROR.png`) }).catch(() => undefined);
      results.push({ signal, error: String(err) });
    }
  }

  console.log(`\n${"=".repeat(60)}`);
  console.log("DONE — ALL SIGNALS COMPLETE");
  console.log(`${"=".repeat(60)}`);
  console.log(`Output dir: ${OUTPUT_DIR}`);
  for (const r of results) {
    if (r?.csvPath) {
      const size = fs.existsSync(r.csvPath) ? fs.statSync(r.csvPath).size : 0;
      console.log(`  ${r.signal}: ${r.count} discovered, ${r.totalSaved || 0} saved, CSV ${Math.round(size / 1024)}KB`);
    } else {
      console.log(`  ${r?.signal}: ${r?.error || "no CSV"}`);
    }
  }
