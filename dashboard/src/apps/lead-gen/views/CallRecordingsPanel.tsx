...
import type { CallScore, MyPerformanceGrade, SellerMotivationGrade, SellerSentiment } from '../../../api/types'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, minWidth: 90 }}>
      <div style={{
        ...
        border: `2px solid ${active ? '#22c55e' : '#2a2a3e'}`,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            border: `1px solid ${playing ? '#22c55e25' : '#1e1e2e'}`,
          ...
          {audioUrl && (
            <div style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 10, color: '#555', fontVariantNumeric: 'tabular-nums', minWidth: 30 }}>
                {fmtTime(progress)}
              </span>
              <div
                style={{
                  flex: 1, height: 3, borderRadius: 2, background: '#1e1e2e',
              ...
              <span style={{ fontSize: 10, color: '#555', fontVariantNumeric: 'tabular-nums', minWidth: 30, textAlign: 'right' }}>
                {fmtTime(duration)}
              </span>
            </div>
          )}
...
function ListView({ onSelect, onUpload }: { onSelect: (id: number) => void; onUpload: () => void }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 18, color: '#e0e0e0' }}>Seller Call Recordings</h2>
          {stats && (
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input
          ...
          style={{
            ...
            border: '1px solid #2a2a3e', background: '#111118',
      ...
      {isLoading ? (
        <div style={{ color: '#666', padding: 20 }}>Loading recordings...</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {recordings.map((rec) => {
            ...
            const perf = parseGrade<MyPerformanceGrade>(rec.my_performance_json)
            return (
              <div
                key={rec.id}
              ...
              >
                ...
                <div style={{
                  ...
                  background: 'linear-gradient(135deg, #2a2a3e, #3a3a5e)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  border: `2px solid ${rec.call_score ? scoreColor(rec.call_score) + '60' : '#2a2a3e'}`,
                }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  {perf && (
                    <div style={{
                      ...
                      fontSize: 12, fontWeight: 700, color: perf.score >= 7 ? '#22c55e' : perf.score >= 5 ? '#eab308' : '#ef4444',
...
function DetailView({ recordingId, onBack }: { recordingId: number; onBack: () => void }) {
  ...
  const gradeMutation = useMutation({
    mutationFn: () => hermesClient.callRecordings.grade(recordingId),
  })
  ...
  if (isLoading || !rec) {
    return <div style={{ color: '#666', padding: 40, textAlign: 'center' }}>Loading...</div>
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h2 style={{ margin: 0, fontSize: 18, color: '#e0e0e0' }}>{rec.seller_name}</h2>
            {rec.call_score && (
              <span style={{
      ...
      {rec.transcript && !rec.my_performance_json && (
        <div style={{ marginBottom: 16, textAlign: 'center' }}>
          ...
          >
            {gradeMutation.isPending ? 'Starting...' : 'Grade This Call'}
          </button>
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          {perf && (
            <div style={{
              ...
              <div style={{
                ...
                marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid #1e1e2e',
              }}>
                <h3 style={{ margin: 0, fontSize: 13, color: '#888', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                  My Performance
                </h3>
                <span style={{
                  padding: '4px 14px', borderRadius: 8,
                  background: perf.score >= 7 ? '#22c55e15' : perf.score >= 5 ? '#eab30815' : '#ef444415',
                  border: `1px solid ${perf.score >= 7 ? '#22c55e30' : perf.score >= 5 ? '#eab30830' : '#ef444430'}`,
                  color: perf.score >= 7 ? '#22c55e' : perf.score >= 5 ? '#eab308' : '#ef4444',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <GradeRow label="Controlled conversation" value={perf.controlled_conversation} />
                <GradeRow label="Uncovered motivation" value={perf.uncovered_motivation} />
                <GradeRow label="Handled objections" value={perf.handled_objections} />
                <GradeRow label="Momentum loss" value={perf.momentum_loss} />
              </div>
              {perf.summary && (
                <div style={{
          ...
          {motivation && (
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <GradeRow label="Core reason for selling" value={motivation.core_reason} />
                <GradeRow label="Emotional vs logical" value={motivation.emotional_or_logical} />
                <GradeRow label="Timeline" value={motivation.timeline} />
              </div>
              {motivation.summary && (
                <div style={{
                  marginTop: 14, padding: '10px 14px', borderRadius: 8,
      ...
      {rec.transcript && (
        <div style={{
          ...
          <h3 style={{
            margin: '0 0 14px', fontSize: 13, color: '#888',
            fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5,
          }}>
            Full Transcript
          </h3>
          <div style={{
            maxHeight: 360, overflow: 'auto', padding: 16, borderRadius: 8,
            background: '#0a0a0f', border: '1px solid #1a1a2e',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            ...
            {!editingAction && (
              <button
                ...
                style={{
                  padding: '3px 10px', borderRadius: 4, border: '1px solid #2a2a3e',
          ...
          {editingAction ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button
                  ...
                  style={{
                    padding: '6px 14px', borderRadius: 5, border: '1px solid #2a2a3e',
                    background: 'transparent', color: '#888', fontSize: 12, cursor: 'pointer',
                  }}
                >Cancel</button>
              </div>
            ...
            <div>
              <div style={{ color: '#ccc', fontSize: 13 }}>{rec.next_action || 'No action set'}</div>
              {rec.next_action_due && (
                <div style={{ color: '#666', fontSize: 12, marginTop: 4 }}>Due: {formatDate(rec.next_action_due)}</div>
              )}
            </div>
          )}
        </div>
...
function GradeRow({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div style={{ fontSize: 11, color: '#555', marginBottom: 3, fontWeight: 500 }}>{label}</div>
      <div style={{ fontSize: 13, color: '#ccc', lineHeight: 1.4 }}>{value}</div>
    </div>
  )
}
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        >Back</button>
        <h2 style={{ margin: 0, fontSize: 18, color: '#e0e0e0' }}>Upload Call Recording</h2>
      </div>
      ...
      <div style={{
        ...
        <div>
          <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Seller Name *</label>
          <input type="text" value={sellerName} onChange={(e) => setSellerName(e.target.value)}
            placeholder="John Smith" style={fieldStyle} />
        </div>
        <div>
          <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Property Address</label>
        ...
        <div>
          <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Date of Call</label>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Next Action</label>
            <input type="text" value={nextAction} onChange={(e) => setNextAction(e.target.value)}
              placeholder="Follow up call" style={fieldStyle} />
          </div>
          <div>
            <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Due Date</label>
            <input type="date" value={nextActionDue} onChange={(e) => setNextActionDue(e.target.value)} style={fieldStyle} />
          </div>
        </div>
        <div>
          <label style={{ fontSize: 12, color: '#666', marginBottom: 6, display: 'block', fontWeight: 500 }}>Notes</label>
