# Codex Handoff — TamperMonkey × Swarm × PropStream (v4)
...
- MONITOR command removed from the protocol. PropStream's monitoring feature has no operator-side UI toggle on Pro tier (verified across 5 UI locations).
- EXPORT command's `format` field deprecated — PropStream exports are always CSV, no picker.
- SKIP_TRACE batch flow updated to use export-schema route instead of modal scraping (faster + more reliable).
- New router gate: `Litigator` flag from PropStream short-circuits the pipeline before outreach.
- Note added on AG-Grid virtualization for the My Properties table.
- Reference docs added: `docs/propstream-ui-reference/README.md` (UI ground truth) and `docs/propstream-ui-reference/export-schema.md` (full export column structure). Codex MUST read both before writing selectors.
...
You are the "hands" of the Swarm inside PropStream. The Swarm has a brain (the agents), a nervous system (Hermes routing messages), and a memory (the data schema). What it doesn't have is hands inside PropStream. You're those hands.
...
**04-propstream-bridge** — your channels
| Channel | Role |
|---|---|
...
**Router gates** (`#strategy-router`). Before any expensive operation (save, skip trace), the lead must pass: SFR detached, ARV $150k–$500k, spread supports $20k+ fee, motivation score above threshold, **Litigator flag from PropStream is "No"**. The Litigator gate was added in v4 — PropStream pre-tags owners with litigation history, and these are filtered out before outreach to avoid TCPA/litigation exposure. If you receive a SAVE or SKIP_TRACE command, the lead has already cleared all five gates.
...
4. Receives structured commands: SEARCH, SAVE, EXPORT, SKIP_TRACE, QUOTA_CHECK, HALT, RESUME, PING. (MONITOR removed in v4 — see §2.6 of `docs/propstream-ui-reference/README.md`.)
...
2. Trigger export (CSV only on this PropStream Pro UI).
...
- `EXPORT` — payload: `{ list_name }`. Returns: parsed records per `export-schema.md`. Note: format is always CSV; there is no `format` field.
- `SKIP_TRACE` — payload: `{ property_ids[], list_name?, prefer_batch_route?: bool }`. For batches of 5+, prefer the export-schema route (save → trace from My Properties toolbar → export → parse) over per-property modal scraping.
- `QUOTA_CHECK` — payload: `{}`. Returns: current PropStream-reported quotas + local counters.
- `HALT` — payload: `{ scope: "all" | "saves" | "exports" | "skip_trace" }`. (Note: no `monitor` scope in v4.)
...
The `monitored_used` field stays in the snapshot for observability (read directly from PropStream's account page), even though the userscript cannot trigger monitoring. If PropStream auto-monitors saved properties as a side effect, this field will tick up on its own and we want to see that.
...
**Prerequisites before milestone 1:** Read `docs/propstream-ui-reference/README.md` and `docs/propstream-ui-reference/export-schema.md` end-to-end. Selectors and field mappings live there. Treat them as authoritative.
...
5. **Single property skip trace.** `SKIP_TRACE` on one property. Verify return shape, verify counter increment, **verify nothing leaks to console or Discord**. Note: per `propstream-ui-reference/§5.3`, the populated Contact Information section structure was unobserved — inspect live during this milestone and document findings in `SELECTORS.md`.
6. **Batch operations (10 items).** Run small batches end-to-end. Verify error handling on partial failures. For SKIP_TRACE batch, prefer the export-schema route (save → trace from toolbar → export → parse).
...
8. **Killswitch.** Verify HALT/all stops everything within 5 seconds and RESUME unsticks it.
9. **Session-expiry recovery.** Manually expire the session in another tab. Verify clean error.
10. **First live batch.** 5 zips, ~50 saves total, ~25 skip traces. Operator watches in real-time. Stop at any anomaly.

(Removed in v4: previous monitor-related milestone steps — MONITOR is dropped. The `lane: "houses"` field still stays in the protocol contract for forward compatibility, but it no longer needs a dedicated launch milestone.)

Only after step 10 passes cleanly should the Swarm be allowed to drive at full target volume.
...
Hermes dispatches commands to the userscript via HTTP. The userscript executes a SEARCH per zip, returns result counts via HTTP, and mirrors a summary to `#propstream-results`. The summary surfaces in `#kpi-intelligence` via Hermes routing. Operator approves. Hermes dispatches SAVE, SKIP_TRACE, and EXPORT commands. The userscript executes and reports. Quota counters update. Skip-trace PII goes only to Hermes, never to Discord. Nothing crashes. Operator goes to sleep, wakes up, and there are 200 new qualified leads in `#queue` ranked by motivation, ready to act on.
