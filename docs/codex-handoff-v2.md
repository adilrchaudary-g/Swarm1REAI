# Codex Handoff — TamperMonkey × Swarm × PropStream (v2)

**To:** Codex
**From:** Adil (operator) + Claude (architect)
**Date:** April 26, 2026
**Version:** 2.0 — supersedes v1
**Goal:** Build a TamperMonkey userscript that turns PropStream's web app into a remote-controllable interface for the Houses lane of the Swarm.

**What changed since v1:**
- Architecture split into shared backbone vs. lane-specific channels (this bridge serves the Houses lane only)
- Primary transport is now Hermes HTTP endpoint (Option C in v1) — confirmed available
- Dedicated channels for the bridge: `#house-propstream-commands` (inbound) and `#house-propstream-results` (outbound)

---

## 1. Mission brief

You are joining a project mid-flight. The operator (Adil) is building an autonomous virtual wholesaling system — software that scouts, scores, and qualifies single-family-residence wholesale deals across the U.S. without him driving every step manually. The architecture is a swarm of specialized agents organized into a **shared backbone + downstream lanes** model, coordinated through a Discord-based operating layer powered by a custom orchestrator bot named **Hermes** (instance: **Alfred**). Hermes has its own HTTP backend — it's not a Discord-only bot.

The data engine is PropStream Pro, accessed through its web app. PropStream does not expose a customer-facing API at the Pro tier, so the bridge between the Swarm and PropStream is a TamperMonkey userscript running in the operator's browser. **That userscript is what you're building.**

You can think of the userscript as the "hands" of the Houses lane. The Swarm has a brain (the agents), a nervous system (Hermes routing messages between channels and agents), and a memory (the data schema). What it doesn't have is hands inside PropStream. You're those hands — but only for the Houses lane. Land has its own (future) data source and its own (future) bridge.

If you're a game design person reading this: the Swarm is the AI director, Hermes is the input router, the userscript is one specific input controller wired into the controller port labeled "houses." A separate controller will eventually plug into the "land" port.
...
### 2.2 The Swarm architecture (corrected from v1)

The Discord server "Agents Trying to not Schizophreniao" is split into three logical layers. **You need to understand this split because it determines which channels you talk to.**

**Layer 1 — Shared Backbone (lane-agnostic).** These channels handle work that's the same whether the asset is a house or a piece of land. Every lead, regardless of lane, flows through these:

| Channel | Role |
|---|---|
| `#ai-hq` | Command layer — priorities, routing decisions, architecture changes, cross-lane coordination |
| `#opportunity-intake` | Lead lifecycle entry point |
| `#lead-enrichment` | Skip trace + owner/property record completion |
| `#strategy-router` | Four-gate filter (asset type, price, spread, motivation) |
| `#follow-up-orchestrator` | Cadence and re-engagement logic |
| `#kpi-intelligence` | Usage dashboards + reporting |
| `#queue` | Priority queue sorted by motivation, capped by capacity |
| `#done-feed` | Closed/dead lead archive |

**Layer 2 — Houses lane (downstream execution).** These channels handle SFR-specific work and are powered by PropStream:

| Channel | Role |
|---|---|
| `#house-list-builder` | Market Selector — ranks zip codes, picks targets |
| `#house-distress-monitoring` | Distress signal ingestion |
| `#house-seller-motivation` | Motivation scoring (0–100) |
| `#seller-response-triage` | Persona classifier |
| `#house-fast-underwriting` | MAO calculation + ARV/repair confidence |
| `#house-outreach` | (out of scope for this build) |
| `#house-buyer-match-dispo` | (out of scope for this build) |
| `#house-propstream-commands` | **NEW — Swarm → userscript inbound channel (audit trail)** |
| `#house-propstream-results` | **NEW — userscript → Swarm outbound channel (audit trail)** |

**Layer 3 — Land lane (downstream execution).** Parallel structure to Houses but for vacant land. **Out of scope for this build** — but design with knowledge that a sister bridge will be built later. Don't hardcode anything that assumes "PropStream is the only data source" at the architectural level.

### 2.3 The lane scoping rule (must follow)

The PropStream userscript belongs to the Houses lane. That means:

- **Commands originate from Houses-lane agents only.** The shared backbone doesn't drive PropStream directly — it consumes outputs from the Houses lane via `#opportunity-intake`. If you ever receive a command that appears to come from a land-lane channel, reject it as `INVALID_COMMAND` with reason "out of lane scope."
- **Halt commands from `#ai-hq` are honored regardless of lane.** Operator override is global. Operator commands always supersede agent commands.
- **Results flow up from Houses lane → shared backbone.** The userscript writes to `#house-propstream-results`. Hermes routes from there into `#opportunity-intake` and `#lead-enrichment` for the rest of the pipeline.

### 2.4 The agent framework (your inputs)
...
**Regulatory pre-filter** (Houses lane, Market Selector). Zips in these states are excluded entirely: SC, IL, OK, KY, PA, VA. Zips in these states are deprioritized: CT, OR, MD, AZ, CA, IA, TN, IN, WI, ND. The Swarm will never tell you to operate in a blocked state. If you somehow receive a command targeting a blocked-state zip, reject as `INVALID_COMMAND` with reason "regulatory blocklist" — defense in depth.

**Zip ranking** (Houses lane, Market Selector). Top 50 zips ranked weekly. You receive a target zip + filter set per command.

**Router gates** (Shared backbone). Before any expensive operation (save, skip trace), the lead must pass: SFR detached, ARV $150k–$500k, spread supports $20k+ fee, motivation score above threshold. If you receive a SAVE or SKIP_TRACE command, the lead has already cleared these.

**Motivation score** (Houses lane). 0–100, computed from distress urgency (0.35), financial pressure (0.25), life event (0.15), engagement (0.15), condition deterioration (0.10), with a freshness decay factor.

**Underwriting** (Houses lane). MAO formula: `(ARV × discount_factor) − repairs − assignment_fee − holding_costs`. Confidence = min of ARV / repair / freshness sub-scores.

### 2.5 The launch plan you're plugging into
...
**Halt thresholds:** at 70% of any quota → warning posted to `#ai-hq`. At 85% → switch to high-confidence-only mode (Swarm-side filtering). At 95% → operator/Swarm sends HALT command for that operation type.
...
3. Mirrors all activity to dedicated Discord channels (`#house-propstream-commands`, `#house-propstream-results`) for audit/observability.
...
10. Provides a kill-switch the operator can trigger from `#ai-hq`.
...
- **Primary transport (both directions):** Hermes HTTP endpoint. Confirmed available. Operator will provide the endpoint URL + authentication scheme on first run; you store via `GM_setValue`.
- **Audit transport (mirroring only):** Discord webhooks posting to `#house-propstream-commands` (inbound mirror) and `#house-propstream-results` (outbound mirror). The bridge does **not** read commands from Discord — Discord is observability only. Hermes is the source of truth.
...
3. **Cross-agent observability.** Other agents in the Swarm (motivation scorer, persona classifier, etc.) can subscribe to `#house-propstream-results` for raw lead data without needing Hermes-side coupling.

### 4.4 Open question for the operator before you start

Confirm with operator:
1. Hermes endpoint URL + auth scheme (bearer token? signed request? mTLS?)
2. Does Hermes support long-polling or do we short-poll?
3. Webhook URLs for the two new Discord channels (operator creates the channels first, then generates webhooks)
...
```javascript
// ==UserScript==
// @name         PropStream × Swarm Bridge (Houses lane)
// @namespace    swarm.wholesaling.houses
// @version      0.1.0
// @description  Bridges the Houses-lane Swarm command queue to PropStream's web UI
...
- `transport/`
  ...
  - `discord.js` — webhook client for audit mirroring
...
4. Parse and forward to Hermes (and mirror summary to `#house-propstream-results`; never mirror full PII payload to Discord).
...
**Critical:** never log skip-trace results to console at any verbosity, never write them to the page DOM, never include them in a screenshot if you build a debug screenshot feature, never mirror them to Discord channels at any verbosity. They are PII with TCPA implications. They go: PropStream UI → in-memory variable → outbound HTTPS to Hermes → nowhere else.
...
If PropStream's reported number drifts more than 5% from your local counter, post a reconciliation alert to `#ai-hq` (via webhook) and trust PropStream's number.
...
### Lane tag
Every record emitted by this bridge is tagged `lane: "houses"`. The future land bridge will tag `lane: "land"`. Shared backbone agents route on this field.
...
The `lane` field is mandatory. Reject any inbound envelope without it or with a value other than `"houses"`.
...
- `INVALID_COMMAND` — payload didn't validate (includes lane mismatch, regulatory blocklist hits)
- `EXECUTION_TIMEOUT` — operation took too long
- `OUT_OF_LANE_SCOPE` — command targeted a non-houses operation
...
Every 60 seconds while idle, send a `heartbeat` envelope to Hermes with: script version, uptime, last successful command timestamp, current quota snapshot, queue depth. Mirror a compact version (one-line) to `#house-propstream-results` every 10 minutes — not every minute, to avoid channel spam.
...
on every command of cost-bearing type:
  ...
  if local / soft_cap crosses 70%, 85%, 95% threshold:
    emit threshold-crossing alert to Hermes AND mirror to #ai-hq
```

Local counters reset at the start of each PropStream billing cycle. Build in a manual reset command so the operator can flush at the right time.
...
- On `DOM_SELECTOR_MISSING`, fall back to a "page snapshot" mode: capture the page's outer HTML structure (sanitized — no PII), post it to `#ai-hq`, halt the relevant operation. Operator can adjust selectors in `SELECTORS.md` and resume.
...
Every command's `lane` field is checked first. Wrong lane → instant `OUT_OF_LANE_SCOPE` rejection, no PropStream interaction. This protects against future bugs in Hermes that might cross-wire lanes.
...
- **Discord webhook URLs:** stored via `GM_setValue`. Operator creates the channels and webhooks first, then pastes URLs into the panel.
...
- **Killswitch:** operator posts `@Alfred kill-bridge` in `#ai-hq`. Hermes forwards a HALT/all command. The userscript receives it, sets a master kill flag in `GM_setValue`, refuses all further commands until `RESUME/all`. Kill flag survives page reloads.
...
1. **Skeleton + heartbeat.** Userscript loads on PropStream, posts heartbeat to Hermes (HTTP) every 60s, mirrors compact heartbeat to `#house-propstream-results` every 10min. No PropStream interaction yet. Verify both channels receive heartbeats.
...
7. **Quota threshold simulation.** Manually set local counter near 70%, 85%, 95%. Verify alerts fire to both Hermes and `#ai-hq`.
...
2. `README.md` — install instructions, first-run setup (Hermes endpoint, auth token, both webhook URLs, operator timezone), control panel walkthrough, troubleshooting.
...
1. Hermes HTTP endpoint URL + authentication scheme (bearer / signed / mTLS).
2. Does Hermes support long-polling for command delivery, or short-poll only?
3. Confirm operator has created `#house-propstream-commands` and `#house-propstream-results` and generated webhook URLs for each.
4. Operator timezone (for the 8am–11pm operating window).
5. Confirm browser is Chrome (Edge / Brave / Arc all work too).
6. Does Hermes already enforce lane discipline upstream, or is the userscript's `lane` check the first/only check?

---

## 15. What "done" looks like

The operator posts in `#ai-hq`:

> `@Alfred run market-selector → top 5 zips → list builder dry run`

Hermes dispatches commands to the userscript via HTTP. The userscript executes a SEARCH per zip, returns result counts via HTTP, and mirrors a summary to `#house-propstream-results`. The summary surfaces in `#kpi-intelligence` via Hermes routing. Operator approves. Hermes dispatches SAVE, SKIP_TRACE, and MONITOR commands. The userscript executes and reports. Quota counters update. Skip-trace PII goes only to Hermes, never to Discord. Nothing crashes. Operator goes to sleep, wakes up, and there are 200 new qualified leads in `#queue` ranked by motivation, tagged `lane: houses`, ready to act on.

That's the system. You're building the part that makes the Houses lane possible.
